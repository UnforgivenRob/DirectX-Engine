...
private:
	...
	static HANDLE myCreateFile(LPCSTR filename, DWORD access, DWORD creationdisposition, DWORD attributes );
	static BOOL myReadFile(HANDLE hfile, LPVOID lpBuffer, DWORD numBytesToRead, LPDWORD bytesRead);
	static BOOL myWriteFile(HANDLE hfile, LPVOID lpBuffer, DWORD numBytesToWrite, LPDWORD bytesWritten);
	static DWORD File::mySetFilePointer(HANDLE hfile, int offset, DWORD whereInFile);
};
