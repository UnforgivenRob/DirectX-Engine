...
extern std::vector<TGA_Image> textures;
...
void FindAndDisplayTextureInfoByProperty(FbxProperty pProperty, bool& pDisplayHeader, int pMaterialIndex){
    ...
    if( pProperty.IsValid() )
    {
		...
		textures.reserve(lTextureCount);
		...
		for (int j = 0; j < lTextureCount; ++j)
		{
			...
			if (lLayeredTexture)
			{
                ...
                for(int k =0; k<lNbTextures; ++k)
                {
                    ...
                    if(lTexture)
                    {
                        ...
                        //DisplayTextureInfo(lTexture, (int) lBlendMode); //RWW
						printf("X:%s",lTexture->GetName());
                    }
			...
			else
			{
                ...
                if(lTexture)
                {
					...
					printf("        %s\n", ((FbxFileTexture*)lTexture)->GetFileName()); //RWW
					printf("        %s\n", lTexture->GetName());
					TGA_Image img = TGAReader::readTGA(((FbxFileTexture*)lTexture)->GetFileName(), lTexture->GetName());
					if( img.width > 0 )
					{
	...
	printf("NumTexts: %d", textures.size());
    }//end if pProperty
}
